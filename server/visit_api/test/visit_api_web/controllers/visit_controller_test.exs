defmodule VisitApiWeb.VisitControllerTest do
 ...
 # use VisitApi.Accounts
  ...
  describe "create visit" do
    ...
    test "renders errors user does not have enough minutes to request visit", %{conn: conn, user: %User{id: user_id} = user} do
      create_attrs = %{
  ...
  #     conn = put(conn, Routes.visit_path(conn, :update, visit), visit: @update_visit_attrs)
  ...
  #     conn = put(conn, Routes.visit_path(conn, :update, visit), visit: @invalid_visit_attrs)
  #     assert json_response(conn, 422)["errors"] != %{}
  #   end
  # end
